...
void *htab_search_value_thread(void *arg)
{
    ...
    for (size_t i = data->start; i < data->end; ++i)
    {
        for (item_t *bucket = data->h->buckets[i]; bucket != NULL; bucket = bucket->next) {
            if (bucket == NULL) {
                printf("Null bucket at index %zu\n", i);
                fflush(stdout);
                continue;
            }
...
void htab_search_value(htab_t *h, int search)
{
    ...
    size_t buckets_per_thread = h->size / NUM_THREADS;

    for (int i = 0; i < NUM_THREADS; i++) {
        thread_data[i].h = h;
        thread_data[i].search = search;
        thread_data[i].start = i * buckets_per_thread;
        if (i == NUM_THREADS - 1) {
            thread_data[i].end = h->size;
        } else {
            thread_data[i].end = (i+1)*buckets_per_thread;
        }

        pthread_create(&threads[i], NULL, htab_search_value_thread, &thread_data[i]);
    }
